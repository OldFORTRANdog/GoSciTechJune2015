...
void loop(){
  ...
  motorLeft->setSpeed(map(rightLight,0,1020,100,200));  //maps the variable of the amount of light between 0-500 and changes it to a speed of 100-200. If flip flopped, it turns toward dark. 
  motorRight->setSpeed(map(leftLight,0,1020,100,200));
  byte direction;
  if (rightLight >= 500 && leftLight<=500) {
    motorRight->setSpeed(50);
    motorLeft->setSpeed(150);
    motorRight->run(BACKWARD);
    motorLeft->run(FORWARD);
  }
  else if (leftLight >= 500 && rightLight <=500) {
