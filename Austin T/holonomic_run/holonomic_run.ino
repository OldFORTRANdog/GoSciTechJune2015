...
void loop(void){
  ...
  if( duration > 0 ) {
    ...
    const float sqrt3o2 = 1.0*sqrt(3)/2;  // square root of three over two = 
    //STEP 3. Find wheel vectors
    float wB = -vx ; 
    float wL = 0.5*vx - sqrt3o2 * vy; 
    float wR = 0.5*vx + sqrt3o2 * vy; 
   
    byte wB_speed = (byte) map(abs(wB), 0, 100, 10, 250);
    byte wL_speed = (byte) map(abs(wL), 0, 100, 0, 255);
    byte wR_speed = (byte) map(abs(wR), 0, 100, 20, 255);
    ...
    */
    motorBack->run((wB > 0) ? 1: 2 ); // if statement within a function call.
    motorLeft->run((wL > 0) ? 2: 1 );
