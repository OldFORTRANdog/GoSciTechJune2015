...
const byte FORWARD_SPEED_2 = 115;
...
void loop() {
  ...
  motorRight->setSpeed(FORWARD_SPEED_2);
  ...
  else if (Distance_inches(sonic.ping()) <= TARGET_DISTANCE_INCHES)
  {
    motorLeft->run(RELEASE);   // So stop power to the motors
    motorRight->run(RELEASE);  // and move to next section of code
    spin(-90, 80, *motorLeft, *motorRight);
    delay(1000);
    Left_Distance = Distance_inches(sonic.ping());
    delay(500);
    spin(-90, 50, *motorLeft, *motorRight);
    motorLeft-> run(RELEASE);
    motorRight->run(RELEASE);
    delay(1000);
    spin(-90, 50, *motorLeft, *motorRight);
    ...
    motorRight->run(RELEASE);
    delay(1000);
    Right_Distance = Distance_inches(sonic.ping());
