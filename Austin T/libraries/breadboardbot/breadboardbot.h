...
const float trackWidth = 9.0; // inches
...
void allStop(int direction, Adafruit_DCMotor motorLeft, Adafruit_DCMotor motorRight) {
  ...
  motorRight.setSpeed(100); // we need Not reset them in other routine.
...
float duration_per_distance( float distance, byte speed) {
  ...
  float dist_per_sec = (0.1804 * float(speed)) - 1.96;// in inches, from data
