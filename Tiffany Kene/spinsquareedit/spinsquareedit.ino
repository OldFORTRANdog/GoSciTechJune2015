...
#define trackWidth  9.125; // inches       the units used everywhere for distance
*/
...
float duration_per_distance( float distance, byte speed) {
  ...
  float dist_per_sec = (0.2426 * float(speed)) - 7.53;// in inches, from data
  int duration = abs(distance) / dist_per_sec;    // needed time in sec
