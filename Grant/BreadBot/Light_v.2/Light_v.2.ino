...
void loop(){
  ...
  Serial.println(rightLight);        // three statements to get our output.
  ...
  motorLeft->setSpeed(map(rightLight,200,900,50,155));
  motorRight->setSpeed(map(leftLight,200,900,50,155));
...
void twoloop() {
  ...
  motorLeft->setSpeed(map(rightLight,200,900,100,200));
  motorRight->setSpeed(RELEASE);
  motorRight->run(RELEASE);
  delay(leftLight =rightLight);
  }
  if (leftLight > rightLight) {
  motorLeft->setSpeed(RELEASE);
  motorLeft->run(RELEASE);
  motorRight->setSpeed(map(leftLight,200,900,100,200));
  delay(rightLight = leftLight);
