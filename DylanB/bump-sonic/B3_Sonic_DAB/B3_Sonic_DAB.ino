...
// Sonic sensor
const float TARGET_DISTANCE_INCHES = 20; 
...
//  Serial.println(Distance_cm(ping_milli));
  ...
  /* It must have been the sonar sensor */
    ...
    while(Distance_inches(sonic.ping())<=TARGET_DISTANCE_INCHES){
      if(Distance_inches(sonic.ping())>TARGET_DISTANCE_INCHES/2){ // the robot is some distance away
        motorLeft->setSpeed(BACKWARD_SPEED);
        motorRight->setSpeed(BACKWARD_SPEED);
        motorLeft->run(FORWARD);   // rotate until an a distance > the target distance is found
        motorRight->run(BACKWARD);  // then continue
      }else{                                                     //the robot is really close 
        motorLeft->setSpeed(BACKWARD_SPEED/2); // Slowly back up and turn to right
        motorRight->setSpeed(BACKWARD_SPEED);  //if distance > the target distance, continue
        motorLeft->run(BACKWARD);
        motorRight->run(BACKWARD);
      }
      float dist=Distance_inches(sonic.ping());
      for(int i = 0; i<min(int(dist),10);i++){
        Serial.print("I");
      }
      if(float(int(dist))+0.5<dist){
        Serial.println("i");
      }else{
        Serial.println("");
      }
