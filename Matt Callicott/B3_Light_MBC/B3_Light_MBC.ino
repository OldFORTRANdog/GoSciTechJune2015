...
void loop(){
  ...
  int brightness = map(average,0,500,0,255);
  analogWrite(ledPin, brightness); // Changes led brightness due to light values
  ...
  motorLeft->setSpeed(map(rightLight,0,500,80,235));
  motorRight->setSpeed(map(leftLight,0,500,100,255));
  byte direction;
  if (PHOTOTAXIS) {
    direction = FORWARD;
  }
