...
const byte BACKWARD_SPEED = 30;  // and backup/turn speed
...
float leftPingDistance;
float rightPingDistance;
...
void loop(){
  ...
  /* It must have been the sonar sensor */
    ...
    motorLeft->run(RELEASE);
    motorRight->run(RELEASE);
    // Looks both wasy befor moving
    /*delay(1000);    
    motorLeft->run(BACKWARD);   // So stop power to the motors
    motorRight->run(BACKWARD);  // and move to next section of code
    delay(100);
    spin(-90, 20, *motorLeft, *motorRight); // Turns the bot to the left to check distance
    delay(1000);
    leftPingDistance = Distance_inches(sonic.ping()); // Gets distance of left side 
    spin(90, 20, *motorLeft, *motorRight); // Turns the bot to the origional angle
    delay(1000);
    spin(90, TESTSPEED, *motorLeft, *motorRight); // Turns the bot to the right side to check distance
    delay(1000);
    rightPingDistance = Distance_inches(sonic.ping()); // Gets distance of right side
    if (rightPingDistance > leftPingDistance) { // Checks if the right distance is greater than the left distance
      digitalWrite(rightLed, HIGH);
      delay(1000);
      digitalWrite(rightLed, LOW);
      return; // Returns motors to forward 
    }
    else {
      digitalWrite(leftLed, HIGH);
      delay(1000);
      spin(-90, TESTSPEED, *motorLeft, *motorRight); // Turns bot to the origional angle
      delay(1000);
      spin(-90, TESTSPEED, *motorLeft, *motorRight); // Turns bot to the left
      digitalWrite(leftLed, LOW);
      return; // Returns motors to forward
    }*/
  }
