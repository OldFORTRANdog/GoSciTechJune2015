...
// Bump behavior
const byte FORWARD_SPEED = 60;   // Define normal speeds 100
...
const byte BACK_MOTOR_PORT = 2; // NEWOMNI
// Create pointers to motor control objects
Adafruit_DCMotor *motorLeft = AFMS.getMotor(LEFT_MOTOR_PORT);
Adafruit_DCMotor *motorRight = AFMS.getMotor(RIGHT_MOTOR_PORT);
Adafruit_DCMotor *motorBack = AFMS.getMotor(BACK_MOTOR_PORT); //NEWOMNI
...
void loop() {
  // Stops robot if bumpers are hit
  ...
  motorBack->setSpeed(50); //NEWOMNI

  if (leftIR < brightLeft ) {          // Too far right, stop left motor
    motorBack->run(BACKWARD); //NEWOMNI
  }
  else if (rightIR > darkRight) {
    motorBack->run(FORWARD);       // Too far right, stop RIGHT motor NEWOMNI
  }
  motorLeft->run(FORWARD);
  motorRight->run(FORWARD);
  delay(20);
