...
void loop(void){
  ...
  for ( byte leg = 1; leg <= 1; leg++ ) {
    drive(36., TESTSPEED, *motorLeft, *motorRight);// Forward for 10 inches
    spin(-180., TESTSPEED, *motorLeft, *motorRight); // 90 deg. to left
    drive(36, TESTSPEED, *motorLeft, *motorRight);
    allStop(FORWARD, *motorLeft, *motorRight);
    delay(500);  // Pause for 1/2 a second after turning
  }
  delay(3000);
  for ( byte leg = 1; leg <= 0; leg++ ) {
    //drive(10., TESTSPEED, *motorLeft, *motorRight);// Forward for 10 inches
    pivot(-90., TESTSPEED, *motorLeft, *motorRight); // 90 deg. to left
    allStop(FORWARD, *motorLeft, *motorRight);
    delay(500);  // Pause for 1/2 a second after turning
  }
  while(1){}
