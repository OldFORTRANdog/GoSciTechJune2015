...
void loop(){
  ...
  int leftLight = analogRead(RIGHT_PR_PIN);
  int rightLight = analogRead(LEFT_PR_PIN);
  ...
  motorLeft->setSpeed(map(rightLight,0,500,0,255));
  motorRight->setSpeed(map(leftLight,0,500,0,255));
  byte direction;
  if (PHOTOTAXIS) {           
    direction = FORWARD;
  }
