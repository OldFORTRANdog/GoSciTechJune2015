...
void setup(void) {
  ...
  */
  ...
  while (digitalRead(RIGHT_BUMP_PIN) == LOW || lightNum <= 15) { // Get high light values until the pin
  ...
  int darkNum = 0;
  while (digitalRead(RIGHT_BUMP_PIN) == HIGH) {} // Pause until the pin is grounded (our switch)
  while (digitalRead(RIGHT_BUMP_PIN) == LOW || darkNum <= 15) {   // Get dark light values until the pin
    // is no longer grounded
    leftIR = analogRead(LEFT_IR_PIN);
    rightIR = analogRead(RIGHT_IR_PIN);
    Serial.print(leftIR);
...
void loop() {
  ...
  if (leftIR >= brightLeft && rightIR <= darkRight) {
    ...
    spin(15., 80, *motorLeft, *motorRight);       // Too far left, stop LEFT motor
    
    motorLeft->run(RELEASE);
    motorRight->run(RELEASE);
  }
  else if (rightIR > darkRight) {
    spin(-15., 80, *motorLeft, *motorRight);       // Too far right, stop RIGHT motor
