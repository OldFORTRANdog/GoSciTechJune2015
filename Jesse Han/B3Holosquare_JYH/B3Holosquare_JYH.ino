...
void loop(void){
  ...
  if( duration > 0 ) {
    ...
    float wB = -vx;  // Multiply by fudge factor to prevent rotation if needed
    float wL = 0.5*vx - sqrt3o2 * vy; 
    float wR = 0.5*vx + sqrt3o2 * vy; 
   
    byte wB_speed = (byte) map(abs(wB), 0, 100, 0, 100);
    byte wL_speed = (byte) map(abs(wL), 0, 100, 0, 100);
    ...
    */
    ...
    motorLeft->run((wL > 0) ? 2: 1 );  //Edited from ? 1: 2);
