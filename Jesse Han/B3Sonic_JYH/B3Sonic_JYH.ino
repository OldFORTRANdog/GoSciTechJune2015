...
void loop(){
  ...
  else 
  {
    ...
    motorRight->setSpeed(BACKWARD_SPEED);
    float leftDistance = Distance_inches(sonic.ping());
    delay(500);
    spin(90, 80, *motorLeft, *motorRight);
    delay(400);
    spin(90, 80, *motorLeft, *motorRight);
    delay(400);
    float rightDistance = Distance_inches(sonic.ping());
...
/*   float inches_per_sec = 13582.67;  // Equivalent to 345 m/s */
/*   return ping * inches_per_sec * 1e-6 *0.5; */
/* } */

/* float Distance_cm(int ping) { */
/*   float cm_per_sec = 34500.;        // Equivalent to 345 m/s */
